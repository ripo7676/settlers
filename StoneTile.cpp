/*
 * Richard Poulson
 * Definitions for the StoneTile class
 */

#include "StoneTile.h"

StoneTile::StoneTile(const int x_translation, const int y_translation, const int row, const int col, const int number) :
    ResourceTile(x_translation, y_translation, row, col, number) {}
void StoneTile::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(widget);
    QBrush b = painter->brush();
    painter->setBrush(QColor(160, 160, 64, 255));
    QPen pen = QPen();
    pen.setWidthF(1.5);
