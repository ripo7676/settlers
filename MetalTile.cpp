/*
 * Richard Poulson
 * Definitions for MetalTile class
 */

#include "MetalTile.h"

MetalTile::MetalTile(const int x_translation, const int y_translation, const int row, const int col, const int number) :
    ResourceTile(x_translation, y_translation, row, col, number) {}
void MetalTile::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(widget);
